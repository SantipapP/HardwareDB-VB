...
Public Class FRM_GPU_Data
    ...
    Private Sub FRM_GPU_Data_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Search() 'สร้างฟังก์ชั่นสำหรับเรียกข้อมูล
        gridgpu.DataSource = _QueryAsDataTable(_sql) 'ให้วัตถุแสดงข้อมูลในตารางฐานข้อมูลโดยการเรียกใช้ฟังก์ชั่นพร้อมใส่พารามิเตอร์ที่ได้รับค่ามาแล้ว
    ...
    Private Sub btadd_Click(sender As Object, e As EventArgs) Handles btadd.Click
        FRM_GPU_Insert.ShowDialog() 'แสดงหน้าต่างสำหรับเพิ่มข้อมูลใหม่เมื่อกดปุ่ม
    End Sub

    Private Sub btdelete_Click(sender As Object, e As EventArgs) Handles btdelete.Click
        If _Confirm("ต้องการลบหรือไม่") Then 'แสดงข้อความยืนยันพร้อมกำหนดคำในพารามิเตอร์
            Dim sql As String = "DELETE FROM [tbl_gpu] WHERE gpu_Model = '" & txtModel.Text & "'" 'ประกาศตัวแปรสำหรับก็บคำสั่งลบข้อมูลในตารางฐานข้อมูล
        ...
        End If
    End Sub
    Private Sub gridgpu_CellMouseUp(sender As Object, e As DataGridViewCellMouseEventArgs) Handles gridgpu.CellMouseUp
        txtModel.Text = gridgpu.Rows.Item(e.RowIndex).Cells(1).Value.ToString 'กำหนดให้กล่องข้อความแสดงค่าในตารางฐานข้อมูลเมื่อคลิกในตาราง
        txtBrand.Text = gridgpu.Rows.Item(e.RowIndex).Cells(2).Value.ToString 'กำหนดให้กล่องข้อความแสดงค่าในตารางฐานข้อมูลเมื่อคลิกในตาราง
    End Sub
    ...
    Private Sub btSearch_Click(sender As Object, e As EventArgs) Handles btSearch.Click
        ...
        da = New OleDbDataAdapter($"SELECT * FROM tbl_gpu WHERE GPU_Brand = '" & txtSearch.Text & "'", cn) 'ให้ตัวแปรเก็บคำสั่ง sql สำหรับค้นหาข้อมูล
        da.Fill(dt) 'นำตัวแปรที่เก็บคำสั่งใปใช้กับตัวแปรที่เก็บตารางฐานข้อมูล
        gridgpu.DataSource = dt.DefaultView 'ให้วัตถุแสดงข้อมูลที่อยู่ในตารางที่ผ่านการกรองมาแล้ว
    End Sub
End Class